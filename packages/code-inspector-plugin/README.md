...
<p style="font-size: 16px;">
  <code><del>“帮我把首页左上角那个蓝色按钮、右侧内容区域提交按钮、左侧菜单折叠按钮、改成品牌主色，hover 再浅一点，文字别动。”</del></code> 👉 改成品牌主色、hover 再浅一点
</p>
<h2>Agent Eyes 👀</h2>
...
推荐安装方式：**先下载 `agent-eyes` skill**。  
skill 会先检查项目里是否已安装 `@agent-eyes/agent-eyes`，如果没有，会继续引导或帮助完成插件安装，然后再读取当前选中的页面上下文。
...
如果你不使用 skill，也可以手动安装并配置 `@agent-eyes/agent-eyes`。
...
插件内置了一个可复用 skill，帮助 Agent 先检查项目是否安装 `@agent-eyes/agent-eyes`，并在执行代码修改前请求“当前选中元素上下文”接口。
...
Agent Eyes 基于原项目演进而来，当前仓库地址为 https://github.com/Mrxyy/agents-eyes，感谢原作者和社区贡献者打下的基础。
